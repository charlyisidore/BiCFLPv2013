  ...
  for(unsigned int i = 0; i < data_.getnbCustomer(); i++)
    {
	  ...
	  for(unsigned int j = 0; j < data_.getnbFacility(); j++)
		  ...
		  for(unsigned int k = 0; (k < nbNodeNonDominated) && toKeep; k++)
                    {
		      if(data_.getAllocationObj1Cost(i,j) < tabZ1[k] && 
			 data_.getAllocationObj2Cost(i,j) < tabZ2[k])
                        {
		      ...
		      else if(data_.getAllocationObj1Cost(i,j) > tabZ1[k] && 
			      data_.getAllocationObj2Cost(i,j) > tabZ2[k])
                        {	
  ...
  for(unsigned int i = 0; i < nodes_[indexNode].getSize(); i++)
    {
      ...
      for( iter = nodes_[indexNode - 1].labels_.begin(); 
	   iter != nodes_[indexNode - 1].labels_.end(); 
	   iter++)
	{
	  ...
	  while( (itDest != nodes_[indexNode].labels_.end() ) && 
		 ( (*itDest).getObj1() < (obj1)) )
	    {
	      itDest++;
	    }
