...
//LABELSETTING
long int runLabelSetting(vector<Box*> &vectorBox, Data &data, list<Solution> &allSolution)
{
  ...
  for(itVector = vectorBox.begin(); itVector != vectorBox.end(); itVector++)
    {						
      ...
      for(list<Solution>::iterator its = m->nodes_[m->getRank() - 1].labels_.begin();
	  its != m->nodes_[m->getRank() - 1].labels_.end();
	  its++) {
	(*its).setY_j((*(*itVector)).getId());
      }
